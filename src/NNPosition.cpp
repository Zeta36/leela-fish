...
const char* NNPosition::StartFEN = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
...
/// operator<<(NNPosition) returns an ASCII representation of the position
std::ostream& operator<<(std::ostream& os, const NNPosition& pos) {
...
/// NNPosition::init() initializes at startup the various arrays used to compute
/// hash keys.

void NNPosition::init() {

  Random rng(1070372);

  for (Piece pc : Pieces)
      for (Square s = SQ_A1; s <= SQ_H8; ++s)
          NNZobrist::psq[pc][s] = rng.RandInt<Key>();

  for (File f = FILE_A; f <= FILE_H; ++f)
      NNZobrist::enpassant[f] = rng.RandInt<Key>();

  for (int cr = NO_CASTLING; cr <= ANY_CASTLING; ++cr)
  {
      NNZobrist::castling[cr] = 0;
      Bitboard b = cr;
      while (b)
      {
          Key k = NNZobrist::castling[1ULL << pop_lsb(&b)];
          NNZobrist::castling[cr] ^= k ? k : rng.RandInt<Key>();
      }
  }

  NNZobrist::side = rng.RandInt<Key>();
  for (int i = 0; i < 102/RULE50_SCALE; ++i) {
      NNZobrist::rule50[i] = rng.RandInt<Key>();
  }
  for (int i = 0; i <= 2; ++i) {
      NNZobrist::repetitions[i] = rng.RandInt<Key>();
  }
}

Key NNPosition::full_key() const {
  ...
  return st->key ^ NNZobrist::rule50[rule50] ^ NNZobrist::repetitions[reps];
}

/// NNPosition::set() initializes the position object with the given FEN string.
/// This function is not very robust - make sure that input FENs are correct,
/// this is assumed to be the responsibility of the GUI.

NNPosition& NNPosition::set(const string& fenStr, NNStateInfo* si) {
/*
...
*/
  ...
  std::memset(this, 0, sizeof(NNPosition));
  std::memset(si, 0, sizeof(NNStateInfo));
...
/// NNPosition::set_castling_right() is a helper function used to set castling
/// rights given the corresponding color and the rook starting square.

void NNPosition::set_castling_right(Color c, Square rfrom) {
...
/// NNPosition::set_check_info() sets king attacks to detect if a move gives check

void NNPosition::set_check_info(NNStateInfo* si) const {
...
/// NNPosition::set_state() computes the hash keys of the position, and other
/// data that once computed is updated incrementally as moves are made.
/// The function is only used when a new position is set up, and to verify
/// the correctness of the NNStateInfo data when running in debug mode.

void NNPosition::set_state(NNStateInfo* si) const {
  ...
  for (Bitboard b = pieces(); b; )
  {
      Square s = pop_lsb(&b);
      Piece pc = piece_on(s);
      si->key ^= NNZobrist::psq[pc][s];
  }

  if (si->epSquare != SQ_NONE)
      si->key ^= NNZobrist::enpassant[file_of(si->epSquare)];

  if (sideToMove == BLACK)
      si->key ^= NNZobrist::side;

  si->key ^= NNZobrist::castling[si->castlingRights];
  for (Piece pc : Pieces)
  {
      for (int cnt = 0; cnt < pieceCount[pc]; ++cnt)
          si->materialKey ^= NNZobrist::psq[pc][cnt];
  }
}


/// NNPosition::set() is an overload to initialize the position object with
/// the given endgame code string like "KBPKN". It is mainly a helper to
/// get the material key out of an endgame code.

NNPosition& NNPosition::set(const string& code, Color c, NNStateInfo* si) {
...
/// NNPosition::fen() returns a FEN representation of the position. In case of
/// Chess960 the Shredder-FEN notation is used. This is mainly a debugging function.

const string NNPosition::fen() const {
...
/// NNPosition::slider_blockers() returns a bitboard of all the pieces (both colors)
...
Bitboard NNPosition::slider_blockers(Bitboard sliders, Square s, Bitboard& pinners) const {
...
/// NNPosition::attackers_to() computes a bitboard of all pieces which attack a
/// given square. Slider attacks use the occupied bitboard to indicate occupancy.

Bitboard NNPosition::attackers_to(Square s, Bitboard occupied) const {
...
/// NNPosition::legal() tests whether a pseudo-legal move is legal

bool NNPosition::legal(Move m) const {
...
/// NNPosition::pseudo_legal() takes a random move and tests whether the move is
/// pseudo legal. It is used to validate moves from TT that can be corrupted
/// due to SMP concurrent access or hash position key aliasing.

bool NNPosition::pseudo_legal(const Move m) const {
...
/// NNPosition::gives_check() tests whether a pseudo-legal move gives a check

bool NNPosition::gives_check(Move m) const {
...
/// NNPosition::do_move() makes a move, and saves all information necessary
/// to a NNStateInfo object. The move is assumed to be legal. Pseudo-legal
/// moves should be filtered out before this function is called.

void NNPosition::do_move(Move m, NNStateInfo& newSt, bool givesCheck) {

  assert(is_ok(m));
  assert(&newSt != st);

//  thisThread->nodes.fetch_add(1, std::memory_order_relaxed);
  Key k = st->key ^ NNZobrist::side;

  // Copy some fields of the old state to our new NNStateInfo object except the
  // ones which are going to be recalculated from scratch anyway and then switch
  // our state pointer to point to the new (ready to be updated) state.
  std::memcpy(&newSt, st, offsetof(NNStateInfo, key));
  newSt.previous = st;
  st = &newSt;
  ...
  if (type_of(m) == CASTLING)
  {
      ...
      k ^= NNZobrist::psq[captured][rfrom] ^ NNZobrist::psq[captured][rto];
      captured = NO_PIECE;
  }
  ...
  if (captured)
  {
      ...
      k ^= NNZobrist::psq[captured][capsq];
//      prefetch(thisThread->materialTable[st->materialKey]);
      st->materialKey ^= NNZobrist::psq[captured][pieceCount[captured]];
  ...
  // Update hash key
  k ^= NNZobrist::psq[pc][from] ^ NNZobrist::psq[pc][to];

  // Reset en passant square
  if (st->epSquare != SQ_NONE)
  {
      k ^= NNZobrist::enpassant[file_of(st->epSquare)];
      st->epSquare = SQ_NONE;
  }
  ...
  if (st->castlingRights && (castlingRightsMask[from] | castlingRightsMask[to]))
  {
      ...
      k ^= NNZobrist::castling[st->castlingRights & cr];
      st->castlingRights &= ~cr;
  }
  ...
  if (type_of(pc) == PAWN)
  {
      ...
      else if (type_of(m) == PROMOTION)
      {
          ...
          // Update hash keys
          k ^= NNZobrist::psq[pc][to] ^ NNZobrist::psq[promotion][to];
          st->materialKey ^= NNZobrist::psq[promotion][pieceCount[promotion] - 1]
              ^ NNZobrist::psq[pc][pieceCount[pc]];
      }
...
/// NNPosition::undo_move() unmakes a move. When it returns, the position should
/// be restored to exactly the same state as before the move was made.

void NNPosition::undo_move(Move m) {
...
/// NNPosition::do_castling() is a helper used to do/undo a castling move. This
/// is a bit tricky in Chess960 where from/to squares can overlap.
template<bool Do>
void NNPosition::do_castling(Color us, Square from, Square& to, Square& rfrom, Square& rto) {
...
/// NNPosition::do(undo)_null_move() is used to do(undo) a "null move": It flips
/// the side to move without executing any move on the board.

void NNPosition::do_null_move(NNStateInfo& newSt) {

  assert(!checkers());
  assert(&newSt != st);

  std::memcpy(&newSt, st, sizeof(NNStateInfo));
  newSt.previous = st;
  st = &newSt;

  if (st->epSquare != SQ_NONE)
  {
      st->key ^= NNZobrist::enpassant[file_of(st->epSquare)];
      st->epSquare = SQ_NONE;
  }

  st->key ^= NNZobrist::side;
...
void NNPosition::undo_null_move() {
...
Move NNPosition::get_move() const {
  return st->move;
}


/// NNPosition::key_after() computes the new hash key after the given move. Needed
/// for speculative prefetch. It doesn't recognize special moves like castling,
/// en-passant and promotions.

Key NNPosition::key_after(Move m) const {
  ...
  if (captured)
      k ^= NNZobrist::psq[captured][to];

  return k ^ NNZobrist::psq[pc][to] ^ NNZobrist::psq[pc][from];
}

/// NNPosition::is_draw() tests whether the position is drawn by 50-move rule
/// or by repetition. It does not detect stalemates.

bool NNPosition::is_draw() const {  //--didn't understand this _ply_ parameter; deleting it.
  ...
  NNStateInfo* stp = st->previous->previous;
  int cnt = 0;
...
bool NNPosition::is_draw_by_insufficient_material() const {
...
int NNPosition::repetitions_count() const {
  int end = std::min(st->rule50, st->pliesFromNull);
...
// NNPosition::has_repeated() tests whether there has been at least one repetition
// of positions since the last capture or pawn move.

bool NNPosition::has_repeated() const {
...
/// NNPosition::flip() flips position with the white and black sides reversed. This
/// is only useful for debugging e.g. for finding evaluation symmetry bugs.

void NNPosition::flip() {
...
/// NNPosition::pos_is_ok() performs some consistency checks for the
/// position object and raises an asserts if something wrong is detected.
/// This is meant to be helpful when debugging.

bool NNPosition::pos_is_ok() const {
  ...
  NNStateInfo si = *st;
  set_state(&si);
  if (std::memcmp(&si, st, sizeof(NNStateInfo)))
...
std::string NNPosition::move_to_san(Move m) const {
  Square from = from_sq(m);
  Square to = to_sq(m);
  Piece pc = moved_piece(m);
...
/// NNPosition::move_is_san() takes a pseudo-legal Move and a san as input and
/// returns true if moves are equivalent.
template<bool Strict>
bool NNPosition::move_is_san(Move m, const char* ref) const {
...
Move NNPosition::san_to_move(const std::string& s) const {
  const char* cur = &s[0];
...
void BoardHistory::set(const std::string& fen) {
  ...
  states.emplace_back(new NNStateInfo());
  cur().set(fen, states.back().get());
}

// Only need to copy the 8 most recent positions, as that's what is needed by
// the eval.  We don't need to fixup NNStateInfo, as we will never undo_move()
